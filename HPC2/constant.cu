...
// Space for the kernel in constant memory
__constant__ float const_kern [64];
...
__global__ void singlePixel(float* in, float* out, int width, int kwidth)
{
    int i = (blockIdx.x * blockDim.x) + threadIdx.x;
    int j = (blockIdx.y * blockDim.y) + threadIdx.y;
    float total = 0;
    int bwidth = (kwidth/2)*2;
    for (int v=0; v<kwidth; v++)
    {
        for (int u=0; u<kwidth; u++)
        {
            total += in[(j+v)*(width+bwidth) + (i+u)] * const_kern[v*kwidth + u];
        }
    }
...
void runConvolution(int argc, char **argv)
{
    ...
    checkCudaErrors(cudaMemcpyToSymbol (const_kern, kernel, sizeof(float)*kwidth*kwidth ));
    ...
    if (strcmp(overhead,"false")==0) singlePixel<<<numBlocks,threadsPerBlock>>>(cuArray, dData, width, kwidth);
