...
void runConvolution(int argc, char **argv)
{
    ...
    int half = kwidth/2;
    int bwidth = half*2;
    float * temp = (float *)malloc((width+bwidth)*(height+bwidth)*sizeof(float));
          ...
          for (int x=0; x<width; x++)
          {
              ...
              for (int v=0; v<kwidth; v++)
              {
                  for (int u=0; u<kwidth; u++) {
                    total += in[(y+v)*(width+bwidth) + (x+u)] * kernel[v*kwidth + u];
                  }
              }
              out[y*width + x] = clamp(total,0.0f,1.0f);
          }
      }
    }
